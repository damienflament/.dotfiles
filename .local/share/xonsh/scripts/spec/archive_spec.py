...
def describe_command_archive():
    ...
    def it_shows_help_screen(command):
        ...
        (
            assert_that(command("archive --help"))
            .succeeds()
            .and_stdout()
    ...
    def it_checks_that_the_directory_exists(command, directory: Path):
        ...
        (
            assert_that(command("archive", directory))
            .fails()
            .and_stderr()
            .is_equal_to(f"erreur: le répertoire {directory} n'existe pas.")
        )

    def it_checks_the_directory_is_a_directory(command, file: Path):
        """vérifie que le répertoire est bien un répertoire"""
        (
            assert_that(command("archive", file))
            .fails()
            .and_stderr()
            .is_equal_to(f"erreur: {file} n'est pas un répertoire.")
        )

    def it_makes_the_archive_and_displays_its_name(command, directory: Path):
